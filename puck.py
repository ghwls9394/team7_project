...
class Puck:
    ...
    def round_reset2(self, player):#라운드가 끝났을 때 퍽2가 라운드에서 패배한 플레이어 코트쪽에
        #위치하도록 해주는 메서드
        if player == 1:
            self.x = 2*const.WIDTH/4
            self.y = const.HEIGHT/2
            self.angle = 0
            self.speed = 0
    ...
    def reset2(self, speed, player):#골을 먹히고 다음 게임이 시작될 때 퍽2의 위치가 중앙에서 시작하여 
        #골을 먹힌 플레이어쪽으로 랜덤한 각도를 가지고 이동하도록 해주는 메서드
        if player == 1:
            self.angle = rand.uniform(-math.pi, 0)+0.1
        elif player == 2:
            self.angle = rand.uniform(0, math.pi)+0.1
        self.speed = speed
        self.x = const.WIDTH / 2
        self.y = const.HEIGHT / 2
    ...
    def end_reset2(self, speed):#사용자가 설정한 라운드 만큼 게임이 진행 된 후 다시 처음부터 게임을 시작하기 
        #위해 reset을 했을 때의 퍽의 위치가 중앙에서 위아래로 반복운동을 하도록 해주는 메서드
        self.angle = 0
        self.speed = speed
        self.x = const.WIDTH / 3 +30
        self.y = const.HEIGHT / 2
