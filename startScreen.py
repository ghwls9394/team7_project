...
def show_info(screen, scr_width, clock):
    while True:
        ...
        line = other_text.render("4. Set the criteria for winning the game (round count, score count) ", True, const.WHITE)
        screen.blit(line, (100, 350))
        line = other_text.render("5. During playtime, game can be paused anytime by pressing SpaceBar", True, const.WHITE)
        screen.blit(line, (130, 390))

        line = other_text.render("6. Obstacles are created randomly for each round, preventing puck movement.", True,
                                 const.WHITE)
        screen.blit(line, (100, 440))
        line = other_text.render("7. The item changes the speed and size of the paddle to two types.", True, const.WHITE)
